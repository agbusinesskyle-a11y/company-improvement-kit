...
When an owner also submits suggestions, include that owner explicitly in both the selected lane requester configuration and the native interview routine. An employee-only list must not accidentally disable owner submissions. Owner approval identity remains separate: submitting a suggestion or confirming its brief never approves implementation. An already ignored Slack message needs a fresh user submission; do not invent or silently replay provider events.
