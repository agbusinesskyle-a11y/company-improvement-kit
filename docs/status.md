...
A real summary exposed a publication-verification defect: Slack returned entity-encoded plain text, leaving the visible message without a verified publication receipt and causing confirmation rejection. The source repair accepts exact content or its exact one-pass encoding, preserving structural and identity checks. A new operator-only recovery reads and verifies the original message and records its receipt without reposting or confirming. A rejected click must be made again after recovery. Both hosted services are deployed and healthy, with all 79 installed runtime files verified on each. The original publication has been reconciled through a provider read without reposting. A subsequent genuine signed confirmation was accepted and the same interview reached submitted. This verifies the repaired confirmation-to-planning handoff; a generated plan, owner approval card and business implementation are separate later stages. No business-app implementation or new owner approval is implied.
...
One genuine persisted interview was picked up under operator direction, and exactly one clarifying question was verified in the original Slack thread. This proves the scoped response/publication path, not an automatic keyword wake after the move. A subsequent genuine prefixed follow-up did wake the destination bot. Its response was held by automatic approval review because an operator stop message ambiguously covered setup and continuing interviews. After scope clarification and review resolution, exactly one next question was verified in the original thread. Automatic wake is now observed; uninterrupted follow-up without review intervention remains unproven. No source tests, practice builds or provider events were replayed. Employee scope, separate owner approval, frozen plans and limits on real application execution remain unchanged.
...
Eleven new routing/access tests and nine affected host tests passed. Provider events were synthetic and database tests used disposable PostgreSQL. Both private hosted services subsequently deployed successfully; all 79 installed runtime files matched the saved source. Both lane inboxes returned 200 with their own credentials and 401 with the other lane credential. Channel membership and both active native routine settings were read independently; the native bot also reported successful read-only inbox calls. The build configuration omitted the packaged verification command, so the operator ran the same hash verifier read-only on each active service and removed temporary access afterward. No deployment or test was repeated for that check. No fresh live employee submission, business-app implementation, merge or release is claimed. Existing approvals remain frozen; configuration changes do not extend them. Historical interviews with old bindings require reconciliation rather than automatic authority changes.
